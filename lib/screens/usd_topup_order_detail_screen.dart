...
/// A single USD top-up order: payment instructions while waiting for the
/// customer to pay the agent, a proof-upload step, live status once the
/// agent reviews it, and an order chat throughout — the Binance-P2P style
/// escrow flow for buying USD with real KES via an Agent.
class UsdTopupOrderDetailScreen extends StatefulWidget {
  final String orderId;
  const UsdTopupOrderDetailScreen({super.key, required this.orderId});

  @override
  State<UsdTopupOrderDetailScreen> createState() =>
      _UsdTopupOrderDetailScreenState();
}

class _UsdTopupOrderDetailScreenState
    extends State<UsdTopupOrderDetailScreen> {
  bool _loading = true;
  ...
  Future<void> _uploadProof() async {
    ...
    try {
      ...
      await ApiService.uploadUsdTopupProof(
  ...
  Future<void> _cancel() async {
    ...
    try {
      await ApiService.cancelUsdTopupOrder(widget.orderId);
      await _load();
    } on ApiException catch (err) {
      if (!mounted) return;
      BybitToast.error(context, err.message);
  ...
  Widget _content(Map<String, dynamic> order) {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          'for ${NumberFormat('#,##0.00').format(amountKes)} KES',
        ...
        if (status == 'PENDING_PAYMENT' || status == 'REJECTED') ...[
          BybitCard(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Send KES ${NumberFormat('#,##0.00').format(amountKes)} via mobile money to $agentName ($agentCode)${agentPhone != null && agentPhone.isNotEmpty ? ' — $agentPhone' : ''}, then upload your payment screenshot below.',
                  style: const TextStyle(
          ...
          Container(
            ...
            child: const Text(
              'Proof submitted. Waiting for the agent to confirm receipt and release your USD — pull to refresh for updates.',
              style: TextStyle(
                color: BybitPalette.accent,
                fontSize: 13,
                fontWeight: FontWeight.w700,
          ...
          Container(
            ...
            child: Text(
              '\$${amountUsd.toStringAsFixed(2)} has been credited to your wallet.',
              style: const TextStyle(
